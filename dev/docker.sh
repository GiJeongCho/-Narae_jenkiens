...
LLM_MODEL_DIR="${LLM_MODEL_DIR:-${SCRIPT_DIR}/LLM/src/resources/model}"
...
IMG_EMBEDDING=your-org/embedding:${ENV_NAME}
...
services:
  ...
  llm_api:
    ...
    environment:
      ...
      REDIS_MASTER_NAME: "mymaster"
    volumes:
      - ${LLM_MODEL_DIR}:/app/src/resources/model:ro
...
apply_firewall_rules() {
  ...
  if command -v ufw >/dev/null 2>&1; then
    ...
    local P=${PORT_BASE}
    for offset in 1 2 3 4 5 6 7 8 9 16 17 112 121 131 132 134 135 136 137 138 139; do
...
compose_action() {
  ...
  case "${ACTION}" in
    ...
    restart)
      ...
      compose_cmd up -d --force-recreate "${SERVICE_TARGETS[@]}"
      compose_cmd ps
      ;;
    list)
      list_services
